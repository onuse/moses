...
use super::*;
use crate::ext4_native::core::constants::EXT4_EXTENTS_FL;
...
impl Ext4Writer {
    ...
    pub(super) fn write_inode(
        ...
        // TODO: Add to transaction for journaling
    ...
    pub(super) fn count_inode_blocks(&self, inode: &Ext4Inode) -> Result<u64, MosesError> {
        if inode.i_flags & EXT4_EXTENTS_FL != 0 {
            self.count_extent_blocks(inode)
        } else {
            // TODO: Count blocks from indirect blocks
            Ok(0)
        }
    }
    
    /// Get all blocks allocated to an inode
    pub(super) fn get_all_inode_blocks(&self, inode: &Ext4Inode) -> Result<Vec<BlockNumber>, MosesError> {
        if inode.i_flags & EXT4_EXTENTS_FL != 0 {
            self.get_extent_blocks(inode)
        } else {
            // TODO: Extract blocks from indirect blocks
            Ok(Vec::new())
        }
    }
