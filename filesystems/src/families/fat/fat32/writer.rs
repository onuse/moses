...
use crate::families::fat::fat32::reader::{Fat32BootSector, Fat32DirEntry};
// use crate::families::fat::fat32::path_resolver::{Fat32PathResolver, DirectoryEntry};
...
const FAT32_BAD: u32 = 0x0FFFFFF7; // Bad cluster marker
...
/// FAT32 Writer with cluster allocation and write capabilities
pub struct Fat32Writer {
    ...
    dirty_fat_entries: HashMap<u32, u32>, // Entries that need to be written
...
impl Fat32Writer {
    ...
    pub fn new(device: Device) -> MosesResult<Self> {
        info!("Opening FAT32 filesystem for writing on device: {}", device.name);
    ...
    pub fn read_fat_entry(&mut self, cluster: u32) -> MosesResult<u32> {
        ...
        let entry = u32::from_le_bytes(entry_bytes) & FAT32_MASK;
    ...
    pub fn write_fat_entry(&mut self, cluster: u32, value: u32) -> MosesResult<()> {
        ...
        // Update cache
        self.dirty_fat_entries.insert(cluster, value & FAT32_MASK);
        self.fat_cache.insert(cluster, value & FAT32_MASK);
    ...
    pub fn flush_fat(&mut self) -> MosesResult<()> {
        for (&cluster, &value) in &self.dirty_fat_entries {
            ...
            for fat_num in 0..self.boot_sector.num_fats {
                ...
                let entry_bytes = (value & FAT32_MASK).to_le_bytes();
    ...
    pub fn allocate_cluster_chain(&mut self, count: u32) -> MosesResult<Vec<u32>> {
        ...
        if prev_cluster != 0 {
            self.write_fat_entry(prev_cluster, FAT32_EOC)?;
        }
    ...
    pub fn extend_cluster_chain(&mut self, last_cluster: u32, additional: u32) -> MosesResult<Vec<u32>> {
        ...
        // Mark the last cluster as end of chain
        self.write_fat_entry(current, FAT32_EOC)?;
    ...
    pub fn write_file_data(&mut self, start_cluster: u32, data: &[u8]) -> MosesResult<()> {
        ...
        if clusters.len() > clusters_needed {
            ...
            self.write_fat_entry(clusters[clusters_needed - 1], FAT32_EOC)?;
    ...
    pub fn create_directory_entry(
        ...
        let mut entry = Fat32DirEntry {
            ...
            write_time: 0,
            write_date: 0,
            first_cluster_lo: (cluster & 0xFFFF) as u16,
            file_size: if attributes & ATTR_DIRECTORY != 0 { 0 } else { size },
        };
        
        // Populate the 8.3 filename from the name parameter
        // Assume name is already in 8.3 format (e.g., "FILENAME.EXT" or "FILENAME   ")
