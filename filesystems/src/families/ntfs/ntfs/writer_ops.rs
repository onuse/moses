...
impl NtfsWriter {
    ...
    pub fn create_file(&mut self, path: &str, initial_size: u64) -> Result<u64, MosesError> {
        info!("Creating file '{}' with initial size {}", path, initial_size);
        ...
        let (parent_path, filename) = { let (p, f) = self.split_path(path); (p.to_string(), f.to_string()) };
        
        // Resolve parent path to MFT number
        let parent_mft = if parent_path == "/" {
            MFT_RECORD_ROOT
        } else {
            // Use the MftReader directly with path resolver
            // TODO: Implement proper reader wrapper for path resolution
            warn!("Nested directories not yet fully supported - defaulting to root");
            MFT_RECORD_ROOT
        };
        
        debug!("Parent directory '{}' resolved to MFT {}", parent_path, parent_mft);
        ...
        self.update_directory_index(parent_mft, mft_record_num, &filename)?;
    ...
    pub fn create_directory(&mut self, path: &str) -> Result<u64, MosesError> {
        ...
        let (parent_path, dirname) = { let (p, d) = self.split_path(path); (p.to_string(), d.to_string()) };
        ...
        debug!("Parent directory '{}' resolved to MFT {}", parent_path, parent_mft);
        // Build MFT record for directory using the builder pattern
        let current_time = windows_time_now();
        let record_size = self.boot_sector.mft_record_size();
        
        let builder = MftRecordBuilder::new(mft_record_num, record_size)
            .as_directory()
            .with_standard_info(
                current_time,
                current_time, 
            ...
            .with_file_name(
                ...
                current_time,
                current_time,
                0,  // Allocated size is 0 for directories
                0,  // Real size is 0 for directories
                FILE_ATTRIBUTE_DIRECTORY
            )?
            .with_index_root(0x30)?;  // $I30 index for directories
        ...
        // Add to parent directory index (root for now)
        self.update_directory_index(parent_mft, mft_record_num, &dirname)?;
    ...
    pub fn delete_file(&mut self, mft_record_num: u64) -> Result<(), MosesError> {
        ...
        // TODO: Need to find parent directory and filename
        // For now, we'll assume root directory
        warn!("Directory index removal not fully implemented - need parent tracking");
    ...
    fn split_path<'a>(&self, path: &'a str) -> (&'a str, &'a str) {
        ...
        if let Some(pos) = normalized.rfind('/') {
            if pos == 0 {
                ...
                // File/dir in subdirectory
                (&normalized[..pos], &normalized[pos + 1..])
            }
        } else {
            // No slash, assume root directory
            ("/", normalized)
        }
    }
}
