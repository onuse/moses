...
use super::journaled_writer::{JournaledNtfsWriter, JournalingConfig};
...
pub struct NtfsRwOps {
    ...
    journaled_writer: Mutex<Option<JournaledNtfsWriter>>,
...
impl NtfsRwOps {
    pub fn new() -> Self {
        NtfsRwOps {
            ...
            journaled_writer: Mutex::new(None),
            device: None,
            write_enabled: false,
...
impl FilesystemOps for NtfsRwOps {
    ...
    fn init(&mut self, device: &Device) -> Result<(), MosesError> {
        ...
        *self.reader.lock().unwrap() = Some(reader);
        ...
        if self.write_enabled {
            ...
            // Create journaled writer if journaling is enabled
            if self.journaling_enabled {
                info!("Initializing NTFS journaled writer");
                let journal_config = JournalingConfig::default();
                let journaled = JournaledNtfsWriter::new(writer, journal_config);
                *self.journaled_writer.lock().unwrap() = Some(journaled);
            } else {
    ...
    fn write(&mut self, path: &Path, offset: u64, data: &[u8]) -> Result<u32, MosesError> {
        ...
        // Use journaled writer if available, otherwise fall back to regular writer
        if self.journaling_enabled {
            ...
            // Use the journaled write operation
            let bytes_written = journaled.write_file_data(mft_record, offset, data)?;
            ...
            // Use the regular write operation
