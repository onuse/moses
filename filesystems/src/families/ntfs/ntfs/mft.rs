...
impl MftRecord {
    ...
    pub fn get_attribute_list_entries(&mut self) -> Option<Vec<crate::families::ntfs::ntfs::attribute_list::ParsedAttributeListEntry>> {
        ...
        if let Some(attr) = self.find_attribute(ATTR_TYPE_ATTRIBUTE_LIST) {
            match attr {
                AttributeData::Unknown(data) | AttributeData::Data(data) => {
                    ...
                    crate::families::ntfs::ntfs::attribute_list::parse_attribute_list(&data).ok()
                }
                _ => None
            }
        } else {
