...
pub struct Ext4Writer {
    ...
    /// Journal (if enabled)
    journal: Option<std::sync::Arc<Jbd2Journal>>,
    /// Journal transaction manager
    journal_trans: Option<JournalTransaction>,
    /// Block size
    block_size: u32,
    /// Inode size
    inode_size: u32,
...
impl Ext4Writer {
    ...
    fn calculate_inode_block(&self, inode_num: u32) -> Result<u64, MosesError> {
        ...
        // Calculate the block for this inode
    ...
    /// Read an inode directly from device (static helper for initialization)
    fn read_inode_internal(
        device: &Device,
        superblock: &Ext4Superblock,
        inode_num: u32,
    ) -> Result<Ext4Inode, MosesError> {
        use std::fs::File;
        use std::io::{Read, Seek, SeekFrom};
        use std::path::PathBuf;
        
        // Calculate which group the inode is in
        let group = (inode_num - 1) / superblock.s_inodes_per_group;
        let index = (inode_num - 1) % superblock.s_inodes_per_group;
        
        // Read group descriptor to find inode table
        let block_size = superblock.s_block_size();
        let gdt_block = if block_size == 1024 { 2 } else { 1 };
        
        // Open device using mount point or device name
        ...
        let gd_size = std::mem::size_of::<Ext4GroupDesc>();
        let gd_offset = gdt_block * block_size + group as u32 * gd_size as u32;
        
        file.seek(SeekFrom::Start(gd_offset as u64))
            .map_err(|e| MosesError::Other(e.to_string()))?;
        
        let mut gd_bytes = vec![0u8; gd_size];
        file.read_exact(&mut gd_bytes)
            .map_err(|e| MosesError::Other(e.to_string()))?;
        
        let group_desc = unsafe {
            std::ptr::read_unaligned(gd_bytes.as_ptr() as *const Ext4GroupDesc)
        };
        ...
        let inode_size = superblock.s_inode_size as u64;
        let inode_offset = inode_table_block * block_size as u64 + index as u64 * inode_size;
        ...
        file.read_exact(&mut inode_bytes)
            .map_err(|e| MosesError::Other(e.to_string()))?;
    ...
    pub fn new(device: Device) -> Result<Self, MosesError> {
        ...
        let (journal, journal_trans) = if enable_journal {
            // Read journal inode (typically inode 8)
            let journal_inode = Self::read_inode_internal(&device, &superblock, EXT4_JOURNAL_INO)?;
            
            // Create journal device
            let journal_device = Box::new(
                crate::families::ext::ext4_native::journal::device::InodeJournalDevice::new(
                    device.clone(),
                    journal_inode,
                    block_size,
                )?
            ...
            let journal_config = JournalConfig {
                journal_inode: EXT4_JOURNAL_INO,
                journal_device: None,
                journal_blocks: journal_inode.i_blocks_lo / (block_size / 512),
                commit_interval: 5,
    ...
    pub fn create_file(
        ...
        // Start journal transaction if enabled
        let mut journal_handle = if let Some(ref trans) = self.journal_trans {
            Some(trans.begin(5)?)  // Reserve 5 blocks for file creation
        } else {
            None
        };
        ...
        // Add inode to journal if enabled
        ...
        // Commit journal transaction first if enabled
    ...
    pub fn write_file(
        ...
        // Start journal transaction if enabled
        let mut journal_handle = if let Some(ref trans) = self.journal_trans {
            Some(trans.begin(10)?)  // Reserve 10 blocks for this operation
        } else {
            None
        };
