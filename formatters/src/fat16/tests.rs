...
use super::formatter_fixed::Fat16FormatterFixed;
use super::verifier::{Fat16Verifier, VerificationResult};
...
mod tests {
    ...
    async fn format_and_verify(size: u64, label: Option<&str>) -> VerificationResult {
        ...
        // Format the device
        let formatter = Fat16FormatterFixed;
    ...
    fn test_boot_sector_structure() {
        use super::super::verifier::Fat16BootSector;
        use std::mem;
        
        // Verify structure size is exactly 62 bytes (before padding to 512)
        assert_eq!(mem::size_of::<Fat16BootSector>(), 62);
        
        // Verify field offsets match FAT16 specification
        let bs = Fat16BootSector {
            jump_boot: [0; 3],
            oem_name: [0; 8],
            bytes_per_sector: 0,
        ...
        unsafe {
            ...
            assert_eq!(ptr.add(0x0B) as *const u16, addr_of!(bs.bytes_per_sector));
            assert_eq!(ptr.add(0x0D) as *const u8, addr_of!(bs.sectors_per_cluster));
            assert_eq!(ptr.add(0x0E) as *const u16, addr_of!(bs.reserved_sectors));
            assert_eq!(ptr.add(0x10) as *const u8, addr_of!(bs.num_fats));
            assert_eq!(ptr.add(0x11) as *const u16, addr_of!(bs.root_entries));
            assert_eq!(ptr.add(0x16) as *const u16, addr_of!(bs.sectors_per_fat));
            assert_eq!(ptr.add(0x24) as *const u8, addr_of!(bs.drive_number));
            assert_eq!(ptr.add(0x26) as *const u8, addr_of!(bs.boot_signature));
        }
    }
