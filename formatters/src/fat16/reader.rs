...
struct DirEntry {
    ...
    create_date: u16,
    access_date: u16,
    first_cluster_high: u16, // Always 0 for FAT16
    modify_time: u16,
    modify_date: u16,
...
impl Fat16Reader {
    pub fn new(device: Device) -> Result<Self, MosesError> {
        ...
        // Validate FAT16
        let fs_type = String::from_utf8_lossy(&boot_sector.fs_type);
        ...
        let root_entries = boot_sector.root_entries as u32;
        let sectors_per_fat = boot_sector.sectors_per_fat as u32;
    ...
    fn parse_short_name(entry: &DirEntry) -> String {
        let mut name = String::new();
        
        // Parse main name (8 chars)
        for &b in &entry.name {
            if b == 0x20 || b == 0x00 {
                break;
            }
    ...
    fn parse_directory(&self, data: &[u8]) -> Vec<FileEntry> {
        ...
        while i + 32 <= data.len() {
            ...
            let entry = unsafe {
                std::ptr::read_unaligned(entry_bytes.as_ptr() as *const DirEntry)
            };
