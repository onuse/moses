...
# Test script for Moses filesystem mounting on Linux
# Prerequisites: FUSE must be installed
...
# Default values
SOURCE_DEVICE="/dev/sdb1"  # Source device with filesystem
MOUNT_POINT="/mnt/moses"    # Where to mount it
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "Options:"
            echo "  -s, --source DEVICE   Source device (default: /dev/sdb1)"
            echo "  -m, --mount PATH      Mount point (default: /mnt/moses)"
...
echo -e "${CYAN}===============================================${NC}"
echo -e "${CYAN}     Moses Bridge - Filesystem Mount Test${NC}"
echo -e "${CYAN}===============================================${NC}"
echo ""
...
if [[ $EUID -ne 0 ]]; then
   ...
   echo -e "${YELLOW}   Moses may need root privileges to mount filesystems.${NC}"
...
if ! command -v fusermount &> /dev/null; then
    echo -e "${RED}❌ FUSE not found!${NC}"
    echo -e "${YELLOW}   Please install FUSE:${NC}"
    echo -e "${YELLOW}   Ubuntu/Debian: sudo apt-get install libfuse-dev fuse${NC}"
    echo -e "${YELLOW}   Fedora/RHEL: sudo dnf install fuse fuse-devel${NC}"
    echo -e "${YELLOW}   Arch: sudo pacman -S fuse2${NC}"
    exit 1
fi
...
if [[ ! -b "$SOURCE_DEVICE" ]]; then
    ...
    echo -e "${YELLOW}   Please specify a valid block device.${NC}"
    exit 1
fi
...
if [[ $? -eq 0 ]]; then
    ...
    if mountpoint -q "$MOUNT_POINT"; then
        ...
        echo -e "  1. Browse files: cd $MOUNT_POINT"
        echo -e "  2. Copy files: cp $MOUNT_POINT/file.txt /tmp/"
        echo -e "  3. Use any Linux tool to access the filesystem"
        echo ""
        echo -e "${YELLOW}To unmount, run:${NC}"
        echo -e "  sudo moses unmount $MOUNT_POINT"
        echo -e "${YELLOW}Or:${NC}"
        echo -e "  sudo fusermount -u $MOUNT_POINT"
    else
    ...
    echo -e "  3. Check kernel messages: dmesg | tail"
